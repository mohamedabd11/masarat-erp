...
export const bspBillings = pgTable('bsp_billings', {
  ...
  totalSalesHalalas:    integer('total_sales_halalas').notNull().default(0),   // gross ticket sales
  totalRefundsHalalas:  integer('total_refunds_halalas').notNull().default(0), // refund/void amounts
  totalCommissionHalalas: integer('total_commission_halalas').notNull().default(0), // agency commission earned
  netRemitHalalas:      integer('net_remit_halalas').notNull(),   // amount due to IATA/BSP
