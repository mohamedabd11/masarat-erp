// POST /api/travel/credentials/:id/disable
...
export async function POST(
  ...
  try {
    ...
    if (!existing.isActive) return NextResponse.json({ success: true, message: 'بيانات الاعتماد معطّلة بالفعل' });
    ...
    await logAudit({
      agencyId, userId: uid,
      action: 'disable', resource: 'provider_credential', resourceId: credId,
      before: { providerCode: existing.providerCode, label: existing.label, isActive: true },
      after:  { isActive: false },
    });

    return NextResponse.json({ success: true, message: 'تم تعطيل بيانات الاعتماد' });
  } catch (err) {
    if (err instanceof ApiAuthError)  return NextResponse.json({ error: err.message }, { status: err.status });
    if (err instanceof BusinessError) return NextResponse.json({ error: err.message }, { status: err.status });
    console.error(JSON.stringify({ event: 'credential_disable_failed', error: String(err) }));
