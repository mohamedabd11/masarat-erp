...
describe.skipIf(SKIP_IF_NO_DB)('assertPeriodOpen — قاعدة بيانات حقيقية', () => {
  ...
  it('يرفض التاريخ المشوّه (fail-closed — سلوك MED-1)', async () => {
    const db = getTestDb();
    // MED-1: تاريخ غير صالح لا يُمرَّر بصمت بل يُرمى BusinessError 400، حتى لا يُنشر
    // قيد بتاريخ يتعذّر إسناده لفترة محاسبية (مطابق لاختبار الوحدة period-lock.test.ts).
    await expect(
