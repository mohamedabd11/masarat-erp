...
describe('buildInvoiceXml — نسبة الضريبة مشتقّة من القيم الفعلية (MED-6)', () => {
  it('يصدر 15.00 للفاتورة القياسية', () => {
    const { xml } = signFixture();   // subtotal 100,000 / vat 15,000 → 15%
    expect(xml).toContain('<cbc:Percent>15.00</cbc:Percent>');
  });

  it('يشتق نسبة TaxSubtotal من vatAmount/taxableAmount بدلاً من 0.15 الثابتة', () => {
    // TaxSubtotal whose effective rate is 5% must emit 5.00 — the old hardcoded
    // `vatAmount > 0 ? 0.15 : 0` could never produce 5.00 anywhere in the XML.
    const { xml } = signFixture({ subtotalHalalas: 100_000, vatHalalas: 5_000, totalHalalas: 105_000 });
    const taxSubtotal = xml.slice(xml.indexOf('<cac:TaxSubtotal>'), xml.indexOf('</cac:TaxSubtotal>'));
    expect(taxSubtotal).toContain('<cbc:Percent>5.00</cbc:Percent>');
  });
});
