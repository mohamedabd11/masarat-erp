    ...
    >
      <div className="bg-white w-full sm:max-w-xl sm:rounded-2xl rounded-t-2xl max-h-[94dvh] flex flex-col shadow-2xl">
          ...
          <div className="text-center">
            ...
            {vatInfo && (
              <p className="text-[11px] text-slate-400 mt-0.5">
                {isVat
                  ? (isAr ? `فاتورة ضريبية (مرحلة أولى) — ضريبة ${vatPct}%` : `Tax Invoice (Phase 1) — ${vatPct}% VAT`)
        ...
        <form id="direct-invoice-form" onSubmit={submit} className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
          ...
          {/* ── ZATCA Phase 2 notice (VAT-registered only) ─────────── */}
          {isVat && (
            <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-xl bg-amber-50 border border-amber-200">
              <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-amber-500 flex-shrink-0 mt-0.5"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>
              <p className="text-[11px] text-amber-800 leading-relaxed">
                {isAr
                  ? 'الفوترة الإلكترونية عبر هيئة الزكاة (المرحلة الثانية) غير مفعّلة حالياً. سيتم إصدار فاتورة ضريبية ورقية (مرحلة أولى) فقط.'
                  : 'ZATCA Phase 2 e-invoicing is not yet active. A paper-based tax invoice (Phase 1) will be issued.'}
              </p>
            </div>
          )}
